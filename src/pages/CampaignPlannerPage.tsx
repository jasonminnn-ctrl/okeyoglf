...
export default function CampaignPlannerPage() {
  ...
  const handleAdd = async () => {
    ...
    await insertCampaign({
      ...
      design_needs: form.design_needs || null,
      memo: form.memo || null,
            ...
            return (
              <Card key={c.id} className="bg-card/50 border-border/50">
                <CardContent className="py-3 px-4 space-y-2">
                  ...
                  {c.purpose && <p className="text-xs text-muted-foreground">목적: {c.purpose}</p>}
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-3 py-2">
            ...
            <Input placeholder="목적" value={form.purpose} onChange={e => setForm(p => ({ ...p, purpose: e.target.value }))} className="text-sm" />
            ...
            <div className="grid grid-cols-2 gap-2">
              <Input type="date" placeholder="시작일" value={form.start_date} onChange={e => setForm(p => ({ ...p, start_date: e.target.value }))} className="text-xs" />
              <Input type="date" placeholder="종료일" value={form.end_date} onChange={e => setForm(p => ({ ...p, end_date: e.target.value }))} className="text-xs" />
            </div>
